...
const (
	LANG_ZH_CN Language = "zh_CN"
	LANG_EN_US Language = "en_US"
)

func Code(ctx *gin.Context) Lang {
	l := ctx.Request.Header.Get("lang")
	lang := Language(l)
	switch lang {
	case LANG_ZH_CN:
		return zh_CN()
...
type Lang struct {
	...
	RSP_CODE_ACCOUNT_TRADE_PWD_ALREADY_SET_ERROR resp.Base //{Code: -2010, Message: "交易密码不为空"}
