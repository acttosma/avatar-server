...
func (mc AvatarOfVender) CreateTableIfNeeded() bool {
	db := mysql.Helper.Db
	err := db.AutoMigrate(mc)
	return err == nil
}

func (mc AvatarOfVender) Add() (AvatarOfVender, error) {
	db := mysql.Helper.Db
	err := db.Create(&mc).Error
	if err != nil {
		logger.Monitor.Errorf("method entity.AvatarOfVender.Add, error:%+v", err)
		return mc, err
	}
...
func (mc AvatarOfVender) FindById(id int64) (AvatarOfVender, error) {
	db := mysql.Helper.Db
...
func (mc AvatarOfVender) FindByAccountId(accountId int64) ([]AvatarOfVender, error) {
	db := mysql.Helper.Db
	var mcs []AvatarOfVender
...
func (mc AvatarOfVender) FindByMchIdAndStoreId(mchId, storeId, preId int64, idCmpSymbol string, size int) ([]AvatarOfVender, error) {
	db := mysql.Helper.Db
	var mcs []AvatarOfVender
...
func (mc AvatarOfVender) FindByMchIdStoreIdAndMobile(mchId, storeId int64, mobile string) (AvatarOfVender, error) {
	db := mysql.Helper.Db
...
func (mc AvatarOfVender) SearchByMchIdStoreIdAndMobile(mchId, storeId, preId int64, mobileLike, idCmpSymbol string, size int) ([]AvatarOfVender, error) {
	db := mysql.Helper.Db
	var mcs []AvatarOfVender
...
func (mc AvatarOfVender) FindByAccountIdAndStoreId(accountId, storeId int64) (AvatarOfVender, error) {
	db := mysql.Helper.Db
...
func (mc AvatarOfVender) CountByMchIdAndStoreIdWithTimeZone(mchId, storeId int64, startTime, endTime time.Time) (int64, error) {
	db := mysql.Helper.Db
	var count int64
